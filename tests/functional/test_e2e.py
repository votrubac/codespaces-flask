...
from data.game_info import Ship, Ships, Board, GameState
import pytest
import json
from app import app
...
def ids() -> GameIds:
    with app.test_client() as test_client:
        ...
        res = test_client.get(f"/set_board/{game_id}?player_id={player1_id}&ships={ships_dump}")
        assert res.status_code == 200
        res = test_client.get(f"/set_board/{game_id}?player_id={player2_id}&ships={ships_dump}")        
        assert res.status_code == 200
